...
export interface SendResult {
  ...
  report?: SendReport;
  /**
   * Present only for a `localTime` send: it becomes one scheduled job per
   * UTC offset your audience keeps, so there is more to say about it than
   * a single instant.
   *
   * `zone_groups` is how many jobs — bounded by the world's clocks, not
   * by the size of your list. `groups_tomorrow` is the number whose slot
   * today has already gone and so run tomorrow instead; a campaign sent
   * at noon UTC asking for 09:00 will have several, and that is the
   * number worth logging, because "the campaign didn't go" is almost
   * always this.
   */
  strategy?: "recipient_local_time";
  local_time?: string;
  zone_groups?: number;
  /** ISO instants: when the earliest and latest zone jobs run. */
  first_at?: string;
  last_at?: string;
  groups_tomorrow?: number;
}
...
export class Notibase {
  ...
  readonly messages = {
    ...
    send: (
      params: {
        ...
        name?: string;
        /**
         * Restrict the send to devices on these platforms. Omit for all of
         * them; `[]` is refused, because "no platforms" and "every platform"
         * must not be the same value in a report you read later.
         *
         * Devices on the others are not targets rather than skipped ones, so
         * a deliberate iOS-only campaign does not read as a wall of failures.
         * Email and SMS ignore it: they address a person, and a person does
         * not have a platform.
         */
  ...
  /**
   * Behaviour, from your server rather than from a device.
   *
   * `POST /v1/events` has accepted a server key since it was written, and
   * this SDK had no way to call it — so the events that only a backend
   * knows about (a payment settled, a shipment moved, a trial ended) could
   * be sent from a browser or a handset and from nowhere else. Those are
   * exactly the events worth automating on, and exactly the ones a client
   * cannot be trusted to report.
   *
   * An event is three things: a name, whatever properties you attach, and
   * optionally the device it happened on. Reserved names — `install` and
   * `session_start` — additionally feed attribution.
   */
  readonly events = {
    /**
     * One event.
     *
     * `device_id` is optional and usually absent from a server: an event
     * about a person is attached to the person through the device when you
     * have one, and stands on its own when you do not.
     */
    track: (event: {
      name: string;
      properties?: Record<string, unknown>;
      ...
      this.request("POST", "/v1/events", { body: event }),

    /**
     * Up to fifty at once, which is what a backfill or a nightly job
     * actually has. One request, one round trip, and the same automations
     * fire as if they had arrived one at a time.
     */
    trackMany: (events: {
      name: string;
      properties?: Record<string, unknown>;
      ...
      this.request("POST", "/v1/events", { body: { events } }),
  };
